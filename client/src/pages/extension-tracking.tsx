...
import { Search, Filter, ChevronLeft, ChevronRight, Download, Puzzle, Eye, ArrowLeft, Activity } from 'lucide-react';
...
export default function ExtensionTracking() {
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-3">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-semibold text-foreground">Extension Tracking</h2>
                  <p className="text-sm sm:text-base text-muted-foreground">Monitor all generated Chrome extensions and their usage</p>
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-2 text-sm">
              <div className="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></div>
              <span className="text-muted-foreground">Extension Monitor Active</span>
            </div>
          </div>
        </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Generated extensions</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Working extensions</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-red-500">
                  {data?.logs.filter(log => log.generationStatus === 'error').length || 0}
                </div>
                <p className="text-sm text-muted-foreground">Generation errors</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-sm text-muted-foreground">Extension downloads</p>
              </CardContent>
            </Card>
          </div>
            ...
            <CardHeader>
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span>Extension Generation Logs</span>
                </CardTitle>
              ...
              <CardDescription>
                Real-time monitoring of all Chrome extension generations and their details
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[900px]">
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="text-left p-3 sm:p-4 text-sm font-medium text-muted-foreground">Extension</th>
                      ...
                      <th className="text-left p-3 sm:p-4 text-sm font-medium text-muted-foreground">Size</th>
                    ...
                    {filteredLogs.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="p-8 text-center text-muted-foreground">
                          {search ? "No extensions match your search." : "No extensions generated yet. Visit Extension Generator to create one."}
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={log.id} className="hover:bg-muted/20 transition-colors">
                          <td className="p-2 sm:p-4 text-xs sm:text-sm text-foreground">
                            <div className="flex flex-col space-y-1">
                              <span className="font-medium">{log.extensionName}</span>
                              <span className="text-xs text-muted-foreground">v{log.extensionVersion}</span>
                              <span className="text-xs text-muted-foreground font-mono">ID: {log.extensionId.substring(0, 8)}...</span>
                            </div>
                          </td>
                          ...
                          <td className="p-2 sm:p-4 text-xs sm:text-sm text-foreground">
                            <div className="flex flex-col space-y-1">
                              <span>{formatFileSize(log.zipFileSize)}</span>
                              <span className="text-xs text-muted-foreground">{log.downloadCount} downloads</span>
                            </div>
                          </td>
                          <td className="p-2 sm:p-4 text-xs sm:text-sm text-foreground">
              ...
              <div className="p-3 sm:p-4 border-t border-border flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
                <p className="text-xs sm:text-sm text-muted-foreground text-center sm:text-left">
                  Showing {Math.min((currentPage - 1) * logsPerPage + 1, data?.total || 0)}-{Math.min(currentPage * logsPerPage, data?.total || 0)} of {data?.total || 0} extensions
                </p>
